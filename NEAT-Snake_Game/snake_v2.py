...
import numpy as np
import pickle
from helper import plot_abs_winner,plot_scores_gen_wise
...
total_height = 800
n_gen = 100
scores_gen = [0 for x in range(0,n_gen)]
pg.init()
...
def drawGrid(win):
    global total_height, total_width, width, rowsn
...
def run(config_file):
    ...
    winner = p.run(run_game, n_gen)

    # Plot the best scores per generation
    plot_scores_gen_wise(scores_gen)


    # show final stats
    print('\nBest genome:\n{!s}'.format(winner))
    
    # Creating file for the winner model
...
def test_winner(winner, n):
    ...
    for _ in range(n):
        ...
        if True:
            while len(snakes) > 0:
                #pg.time.delay(10) #Turn this on if you want the game to start slower
                
                #clock.tick(50)    #Turn this on to run the display at 50FPS
                ...
                for index, snake in enumerate(snakes):
                    ...
                    for x in range(len(snake.body)):
                        mean = 0
                        if snake.body[x].pos in list(map(lambda z:z.pos,snake.body[x+1:])) or frames[index] >= max_frames:
                            plot_score.append(scores[index])                      
                            frames.pop(index)
                            snacks.pop(index)
                            nets.pop(index)
                            scores.pop(index)
    ...
    def calculate_mean_scores(scores):
        ...
        for i in range(len(scores)):
            ...
            mean_score = cumulative_sum / (i + 1)
            mean_scores.append(mean_score)
        
        return mean_scores

    #plot here
    plot_mean_score = calculate_mean_scores(plot_score)
    plot_abs_winner(plot_score,plot_mean_score)
